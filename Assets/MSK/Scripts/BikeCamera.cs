...
//        }
        ...
        if (Switch == 0)
        {
            ...
            */
			...
			bool onGround  = true;
			if(!BikeManager.instance.bikesContols[data.currentBike].wheels[0].collider.isGrounded && !BikeManager.instance.bikesContols[data.currentBike].wheels[1].collider.isGrounded)
			{
				onGround = false;
			}
//			if(!onGround)
//			{
//				bikeRot -= Time.deltaTime * 2f;
//				bikeRot = Mathf.Max(0f, bikeRot);
//			}
//			else
//			{
			...
			if(!onGround)
			{
				...
				if(onGroundTime > 0.4f)
					bikeRot = Mathf.MoveTowardsAngle(bikeRot, 0f, Time.deltaTime * 25f);
			}
			else
			{
				onGroundTime = 0f;
